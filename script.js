...
function createSpaceScene(canvas) {
    ...
    // A more realistic fake sphere: a flat equirectangular "surface map" (terrain
    // blotches, craters with rim highlights, polar ice caps) is painted once onto an
    // offscreen canvas, then rendered every frame as a series of thin vertical strips
    // whose height follows the circle's silhouette (halfH = r*sin) and whose sampled
    // longitude scrolls with rotation — the classic cylindrical-to-sphere canvas trick.
    // A per-strip light/shadow overlay creates a real moving terminator instead of a
    // flat gradient, which is what actually reads as "footage" rather than a sticker.
    ...
    // Strips are painted onto a small offscreen buffer first, then the whole buffer is
    // composited onto the main canvas through a light blur filter — that single blur pass
    // is what fuses the individual strips into a smooth, seamless sphere instead of a
    // barcode of hard edges, and it's far cheaper than filtering every strip separately.
    function drawPlanet() {
        ...
        // A single continuous gradient (sampled from the same brightness formula the old
        // per-strip fills used) replaces per-strip shading — one smooth terminator instead
        // of N discrete alpha steps, which is what was causing the barcode-like banding.
        const lightSteps = 28;
